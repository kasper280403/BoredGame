 ...
 *
 * <p>Implements {@code DiceObserver} to react to dice updates. Uses a {@code Timeline} animation to
 * visually simulate the dice rolling, displaying interim and final dice face images.
 *
...
public class DiceView extends HBox implements DiceObserver {
  ...
  private KeyFrame getDiceKeyFrame(
      Dice dice, int diceValue, Map<Integer, Image> diceImageMap, double wait) {
    return new KeyFrame(
        Duration.millis(wait),
        e -> {
